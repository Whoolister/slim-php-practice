...
(
    ...
    `role`       ENUM ('BARTENDER', 'CERVECERO', 'COCINERO', 'MOZO', 'SOCIO') NOT NULL,
...
(
    `id`     VARCHAR(5)                                           NOT NULL DEFAULT RIGHT(UUID(), 5) CHECK (CHAR_LENGTH(`id`) = 5),
...
(
    ...
    `status`     ENUM ('PENDIENTE', 'PREPARANDO', 'LISTO') AS (CASE WHEN `start_time` IS NULL THEN 'PENDIENTE' WHEN `end_time` IS NULL THEN 'PREPARANDO' ELSE 'LISTO' END) VIRTUAL,
